...
exports.getPlace = function (req, conn, cb) {
    var sql_usr = "select * from slot_booking_bmtc.bmtc_location";
    conn.query(sql_usr, function (err, result) {
        console.log(err);
        cb(result);
    });
...
exports.getTRIP = function (req, conn, cb) {
    var sql_usr = "select * from slot_booking_bmtc.bmtc_trip";
    conn.query(sql_usr, function (err, result) {
        console.log(err);
        cb(result);
    });
...
exports.bmtcSlotLogin = function (req, conn, cb) {
    var bdobj = JSON.parse(Object.keys(req.body)[0]);
